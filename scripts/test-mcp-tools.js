...
async function runTests() {
  ...
  console.log('2. Testing get_balance (initial - should be 0)...');
  try {
  ...
  // Test 3: Request Airdrop for wallet 1
  console.log('3. Testing request_airdrop (1 SOL)...');
  ...
  // Test 4: Get Balance after airdrop
  console.log('4. Testing get_balance (after airdrop)...');
  ...
  // Test 5: Create Wallet 2 (recipient) - but save wallet1 first
  console.log('5. Creating recipient wallet...');
  let wallet2;
  try {
    // Save wallet1's info - we need to keep it as active for sending
    const wallet1Info = wallet1;

    // Create wallet2 just to get an address
    wallet2 = await moltpay.createWallet();
    console.log(`   ✓ Recipient wallet: ${wallet2.publicKey}`);
  ...
  // Test 6: Send Payment from wallet2 (which is now active and has the airdrop)
  // Actually wallet1 received the airdrop but wallet2 is now active.
  // We need to airdrop to wallet2 since it's the active wallet now
  console.log('6. Requesting airdrop for active wallet (wallet2)...');
  try {
    const airdrop2 = await moltpay.requestAirdrop({ publicKey: wallet2.publicKey, amount: 1 });
    if (airdrop2.success) {
      console.log(`   ✓ Airdrop to wallet2 successful: ${airdrop2.signature}`);
    } else {
      console.log(`   ✗ Airdrop failed: ${airdrop2.error}`);
    }
  } catch (error) {
    console.log(`   ✗ Error: ${error.message || error}`);
  }
  ...
  // Wait for second airdrop
  console.log('   Waiting for airdrop confirmation (5 seconds)...');
  await new Promise(resolve => setTimeout(resolve, 5000));
  console.log();

  // Test 7: Send Payment from wallet2 (active) to wallet1
  console.log('7. Testing send_payment (0.1 SOL from wallet2 to wallet1)...');
  let paymentSignature;
  try {
    const payment = await moltpay.sendPayment({
      to: wallet1.publicKey,
      amount: 0.1,
      token: 'SOL',
  ...
  console.log('8. Testing verify_payment...');
  if (paymentSignature) {
    try {
      const verification = await moltpay.verifyPayment({
        signature: paymentSignature,
        expectedRecipient: wallet1.publicKey,
        expectedAmount: 0.1,
      });
  ...
  console.log('9. Testing get_history (wallet2 - sender)...');
  try {
  ...
  // Test 10: Get Recipient Balance
  console.log('10. Testing get_balance (wallet1 - recipient)...');
  try {
    const balance = await moltpay.getBalance({ publicKey: wallet1.publicKey });
    console.log(`   ✓ Wallet1 SOL balance: ${balance.sol}`);
